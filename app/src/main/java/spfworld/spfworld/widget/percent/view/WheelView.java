...
import spfworld.spfworld.widget.percent.weellister.OnItemSelectedListener;
import spfworld.spfworld.widget.percent.weelutils.WheelAdapter;
...
public class WheelView extends View {
	...
	private static final float CENTERCONTENTOFFSET = 1;// 中间文字文字居中需要此偏移值
	...
	protected void onDraw(Canvas canvas) {
		...
		if (label != null) {
			...
			// 靠右并留出空隙
			canvas.drawText("", drawRightContentStart - CENTERCONTENTOFFSET, centerY, paintCenterText);
		}
		counter = 0;
		while (counter < itemsVisible) {
			canvas.save();
			...
			if (angle >= 90F || angle <= -90F) {
				...
				if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
					...
					canvas.drawText(contentText + AbStrUtil.parseEmpty(label), drawOutContentStart, maxTextHeight,
							paintOuterText);
					canvas.restore();
					canvas.save();
					canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
					canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
					canvas.drawText(contentText + AbStrUtil.parseEmpty(label), drawCenterContentStart,
